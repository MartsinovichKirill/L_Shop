...
export function mustString(v: unknown, field: string): string {
  if (typeof v !== "string" || v.trim().length === 0) {
    throw new AppError(`Field "${field}" must be a non-empty string`);
  }
  return v.trim();
}
...
export function mustInt(v: unknown, field: string, min: number, max: number): number {
  if (typeof v !== "number" || !Number.isInteger(v) || v < min || v > max) {
    throw new AppError(`Field "${field}" must be int in [${min}, ${max}]`);
  }
  return v;
}

export function mustOneOf<T extends string>(v: unknown, field: string, allowed: readonly T[]): T {
  if (typeof v !== "string") throw new AppError(`Field "${field}" must be string`);
  if (!allowed.includes(v as T)) throw new AppError(`Field "${field}" invalid value`);
  return v as T;
}
